...
import { BarChart3, Landmark, Droplets, TrendingUp, Scale, Clock, Printer, ArrowUpRight, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function IndicadoresFinancierosView({
  ...
  const stats = useMemo(() => {
    ...
    accSums.forEach(({ debit, credit, account }) => {
      ...
      // Criterios de Clasificación: 1 = Activos, 2 = Pasivos (23 = Patrimonio), 4 = Pérdidas/Gastos, 5 = Ganancias/Ingresos
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Ratios de Liquidez, Rentabilidad (EBITDA, EBIT), Gestión de Cartera (DSO) y Conciliación del Disponible • {company.name} (RUT: {company.rut})
          </p>
        </div>

        <div className="flex items-center gap-2.5 flex-wrap">
          <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-200 text-xs">
            <label className="font-semibold text-slate-600 text-[11px]">Año Fiscal:</label>
            <select
              value={selectedYear}
              onChange={(e) => setSelectedYear(parseInt(e.target.value))}
              className="bg-white border border-slate-300 rounded font-bold px-2 py-0.5 text-xs text-slate-800 focus:ring-1 focus:ring-slate-700 font-mono"
            >
          ...
          <button
            ...
            className="px-3 py-1 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold rounded-md border border-slate-300 flex items-center gap-1.5 transition-colors shadow-2xs"
          >
      ...
      <div className="bg-slate-900 text-white rounded-lg p-4 shadow-xs border border-slate-800">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3 border-b border-slate-800 pb-2">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-bold text-sm uppercase tracking-wide text-slate-200">
              Análisis del Saldo Disponible Real vs. Contabilidad (Tesorería & Bancos)
            </h4>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div className="bg-slate-800/80 p-3 rounded-md border border-slate-700/80">
          ...
          <div className="bg-slate-800/80 p-3 rounded-md border border-slate-700/80">
            ...
            <div className="text-[11px] font-mono tabular-nums mt-1 space-y-0.5">
              <div className="flex justify-between text-emerald-400">
                <span className="font-sans text-[10px]">(+) Abonos banco:</span>
                <span>+${stats.abonosPendientesContabilizar.toLocaleString('es-CL')}</span>
              </div>
              <div className="flex justify-between text-rose-400">
                <span className="font-sans text-[10px]">(-) Cargos banco:</span>
          ...
          <div className="bg-slate-800 p-3 rounded-md border border-indigo-500/50 shadow-xs">
        ...
        {Math.abs(stats.diferenciaDisponibleVsLibros) > 0 && stats.hasReconciliationData && (
          <div className="mt-3 p-2.5 bg-amber-950/40 border border-amber-800/60 rounded-md text-amber-200 text-xs flex items-center gap-2">
            <AlertCircle className="w-4 h-4 text-amber-400 flex-shrink-0" />
            <span>
              <strong>Diferencia de conciliación detectada:</strong> Existen ${Math.abs(stats.diferenciaDisponibleVsLibros).toLocaleString('es-CL')} de diferencia entre el saldo contable y el banco. Revisa el módulo <em>Conciliación Bancaria</em>.
            </span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs">
        <div className="flex items-center gap-2 mb-3 border-b border-slate-200 pb-2">
          ...
          <h4 className="font-bold text-sm uppercase text-slate-900 tracking-wide">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
          {/* Razón Corriente */}
          <div className="p-3.5 rounded-md border border-slate-200 bg-slate-50/60">
            ...
            <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">
              Por cada $1 de deuda de corto plazo, se cuenta con <strong>${stats.razonCorriente.toFixed(2)}</strong> de respaldo en activos circulantes.
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-md border border-slate-200 bg-slate-50/60">
            <div className="flex justify-between items-start">
              <div>
                <span className="text-xs font-bold text-slate-800 block">Prueba Ácida (Quick Ratio)</span>
            ...
            <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">
              Capacidad de cumplir pasivos inmediatos sin depender de la venta de inventario.
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-md border border-slate-200 bg-slate-50/60">
            ...
            <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">
              Recursos operacionales disponibles para financiar la operación del giro continuo.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          ...
          <div className="p-3 bg-slate-50 rounded-md border border-slate-200">
            <span className="text-[11px] font-semibold text-slate-600 uppercase block">Margen Bruto</span>
            <p className="text-lg font-bold text-slate-900 font-mono tabular-nums mt-1">${stats.margenBruto.toLocaleString('es-CL')}</p>
            <div className="flex justify-between items-center text-[11px] text-slate-600 mt-1">
              <span>Margen %:</span>
              <span className="font-bold text-indigo-700 font-mono tabular-nums">{stats.margenBrutoPct.toFixed(1)}%</span>
            </div>
          </div>

          {/* EBIT */}
          <div className="p-3 bg-slate-50 rounded-md border border-slate-200">
            <span className="text-[11px] font-semibold text-slate-700 uppercase block">EBIT (Res. Operacional)</span>
            <p className="text-lg font-bold text-slate-900 font-mono tabular-nums mt-1">${stats.ebit.toLocaleString('es-CL')}</p>
            <div className="flex justify-between items-center text-[11px] text-slate-600 mt-1">
              <span>Margen EBIT:</span>
              <span className="font-bold font-mono tabular-nums">{stats.margenOperacionalPct.toFixed(1)}%</span>
            </div>
          </div>

          {/* EBITDA */}
          <div className="p-3 bg-slate-50 rounded-md border border-slate-200">
            <span className="text-[11px] font-semibold text-slate-700 uppercase block">EBITDA Operacional</span>
            <p className="text-lg font-bold text-slate-900 font-mono tabular-nums mt-1">${stats.ebitda.toLocaleString('es-CL')}</p>
            <div className="flex justify-between items-center text-[11px] text-slate-600 mt-1">
              <span>Margen EBITDA:</span>
              <span className="font-bold font-mono tabular-nums">{stats.margenEbitdaPct.toFixed(1)}%</span>
            </div>
          </div>

          {/* Utilidad Neta */}
          <div className={`p-3 rounded-md border ${stats.utilidadNeta >= 0 ? 'bg-emerald-50/60 border-emerald-200' : 'bg-rose-50/60 border-rose-200'}`}>
            <span className="text-[11px] font-semibold uppercase block text-slate-700">Resultado Neto Final</span>
            <p className={`text-lg font-bold font-mono tabular-nums mt-1 ${stats.utilidadNeta >= 0 ? 'text-emerald-800' : 'text-rose-800'}`}>
              ${stats.utilidadNeta.toLocaleString('es-CL')}
            </p>
            <div className="flex justify-between items-center text-[11px] text-slate-600 mt-1">
              <span>Margen Neto:</span>
              <span className="font-bold font-mono tabular-nums">{stats.margenNetoPct.toFixed(1)}%</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-3 text-xs">
            <div className="flex justify-between items-center p-3 bg-slate-50 rounded-md border border-slate-200">
              <div>
                ...
                <span className="text-slate-500 text-[11px]">Plazo medio de cobro de facturas a clientes</span>
              </div>
              <div className="text-right">
                <span className="text-2xl font-bold text-indigo-700 font-mono tabular-nums">{stats.diasCalle.toFixed(0)}</span>
                <span className="text-xs text-slate-600 ml-1 font-semibold">días</span>
              </div>
            </div>

            <div className="divide-y divide-slate-100 font-mono tabular-nums">
              <div className="flex justify-between py-1.5 text-slate-600">
                <span className="font-sans">Cuentas por Cobrar Vigentes:</span>
                <span className="font-semibold text-slate-900">${stats.cuentasPorCobrar.toLocaleString('es-CL')}</span>
              </div>
              <div className="flex justify-between py-1.5 text-slate-600">
                <span className="font-sans">Ventas Totales del Período:</span>
                <span className="font-semibold text-slate-900">${stats.ventasTotales.toLocaleString('es-CL')}</span>
              </div>
              <div className="flex justify-between py-1.5 text-slate-600">
        ...
        <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-2xs">
          ...
          <div className="space-y-3 text-xs">
            <div className="grid grid-cols-2 gap-2">
              <div className="p-3 bg-slate-50 rounded-md border border-slate-200">
                <span className="text-[10px] text-slate-500 font-semibold uppercase block">Leverage Financiero</span>
                <p className="text-xl font-bold text-slate-900 font-mono tabular-nums mt-1">{stats.leverage.toFixed(2)}x</p>
                <span className="text-[10px] text-slate-500">Pasivo Total / Patrimonio</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-md border border-slate-200">
                <span className="text-[10px] text-slate-500 font-semibold uppercase block">Razón de Endeudamiento</span>
                <p className="text-xl font-bold text-slate-900 font-mono tabular-nums mt-1">{stats.razonEndeudamiento.toFixed(1)}%</p>
            ...
            <div className="divide-y divide-slate-100 font-mono tabular-nums">
              <div className="flex justify-between py-1.5 text-slate-600">
                ...
                <span className="font-semibold text-rose-800">${stats.pasivoTotal.toLocaleString('es-CL')}</span>
              </div>
              <div className="flex justify-between py-1.5 text-slate-600">
                <span className="font-sans">Patrimonio Neto:</span>
                <span className="font-semibold text-emerald-800">${stats.patrimonioTotal.toLocaleString('es-CL')}</span>
              </div>
            </div>
          </div>
        </div>
